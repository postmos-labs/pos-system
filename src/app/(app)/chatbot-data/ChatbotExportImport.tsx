...
7. solution에는 가맹점 사장님이 직접 할 수 있는 조치만 담는다. 원격 접속, 관리자 모드, 마스터 비밀번호처럼 직원만 할 수 있는 조치가 필요하면 그 앞까지의 확인 절차만 남기고 "원격 지원을 요청하세요"로 끝낸다.
8. 초기화, 포맷, 재설치, 데이터 삭제처럼 데이터가 지워질 수 있는 조작은 solution에 넣지 않는다. 그 자리는 "원격 지원을 요청하세요"로 바꾼다.
9. repeat가 true인 항목은 같은 절차로 해결한 뒤 재발한 기록이다. 같은 문제를 다룬 다른 항목이 있으면 그쪽을 우선하고, 이 항목뿐이면 solution 끝에 "다시 발생하면 원격 지원을 요청하세요"를 붙인다.
