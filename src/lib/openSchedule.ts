...
// 오픈일 기준 사전 점검 신호 — 기술지원팀 "오픈일정 누락 방지" 개선안.
//
// 오픈일이 다가올수록 확인 강도를 높인다. 색은 단순 표시가 아니라 담당자가 그날 해야 하는
// 행동을 가리키는 신호다.
//
//   D-5 ~ D-4  노란색  가맹점 유선 통화 · 변경사항 확인
//   D-3 ~ D-1  빨간색  미처리 사항 · 장비 · 일정 최종 점검
//   D-DAY      파란색  설치·오픈
//
// 발표자료 문구는 "D-5 노란색 / D-3 빨간색"이지만 그대로 두 날짜에만 색을 켜면 D-4·D-2에
// 신호가 꺼져 오히려 놓치게 된다. 구간으로 해석해 경계 사이를 메운다.

export const OPEN_CALL_DAYS = 5; // 이 날부터 노란색(필수 통화)
export const OPEN_FINAL_DAYS = 3; // 이 날부터 빨간색(최종 점검)

export type OpenUrgency =
  | "none" // 오픈일이 없거나, 이미 끝난 건
...
/** 오픈일 판정에 필요한 최소 형태. 설치건 행을 그대로 넘길 수 있다. */
...
/** 오픈일까지 남은 일수. 오늘이면 0, 지났으면 음수. 날짜가 없으면 null. */
export function openDaysLeft(openDate: string | null, today: string = kstToday()): number | null {
  if (!openDate) return null;
  // YYYY-MM-DD를 UTC 자정으로 고정해 빼면 타임존과 무관하게 일수만 남는다.
  const target = Date.parse(`${openDate}T00:00:00Z`);
...
export function openUrgency(source: OpenScheduleSource, today: string = kstToday()): OpenUrgency {
  ...
  if (source.status === "completed" || source.status === "rejected") return "none";
  const daysLeft = openDaysLeft(effectiveOpenDate(source), today);
  if (daysLeft === null) return "none";
  if (daysLeft < 0) return "passed";
  if (daysLeft === 0) return "dday";
  if (daysLeft <= OPEN_FINAL_DAYS) return "final";
  if (daysLeft <= OPEN_CALL_DAYS) return "call";
  return "upcoming";
}

/** 지금 챙겨야 하는 건인지 — 목록의 "오픈 임박" 필터와 건수에 쓴다. */
export function isOpenSoon(source: OpenScheduleSource, today: string = kstToday()): boolean {
  const urgency = openUrgency(source, today);
...
const CHIP_CLASS: Record<Exclude<OpenUrgency, "none">, string> = {
...
const CHIP_HINT: Record<Exclude<OpenUrgency, "none">, string> = {
  upcoming: "",
  call: "필수 통화",
  final: "최종 점검",
  dday: "오픈",
...
/**
 * 목록·카드에 붙일 오픈일 칩. 색을 칠할 이유가 없으면 null을 돌려준다.
 *
 * 색은 행 배경이 아니라 이 칩에만 쓴다 — 설치관리 표는 승인 대기(빨강)와 내 담당건(노랑)이
 * 이미 행 배경을 쓰고 있어, 오픈 신호를 행에 얹으면 둘을 구분할 수 없다.
 */
export function openBadge(
  ...
  const daysLeft = openDaysLeft(effectiveOpenDate(source), today) ?? 0;
  const label = daysLeft === 0 ? "D-DAY" : daysLeft > 0 ? `D-${daysLeft}` : `D+${-daysLeft}`;
  return {
    urgency,
    daysLeft,
    label,
    hint: CHIP_HINT[urgency],
    className: CHIP_CLASS[urgency],
  };
}
